...
function ManagePrescriptionsPage() {
  ...
  useEffect(() => {
    ...
    const fetchMedications = async () => {
      try {
        ...
        setLoading(false); // ← обов'язково, навіть якщо помилка
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("/prescriptions", {
        ...
        prescribedById: parseInt(form.prescribedById),
  ...
  onChange={(selected) => setForm((prev) => ({ ...prev, clientId: selected?.value }))}
...
/>
  ...
  onChange={(selected) => setForm((prev) => ({ ...prev, medicationId: selected?.value }))}
...
/>
        ...
        <div className="mb-4">
          <input
            ...
            onChange={(e) => setForm({ ...form, allowedUses: e.target.value })}
          ...
          <tbody>
            {prescriptions.map((prescription) => (
              <tr key={prescription.id} className="border-b">
                <td className="p-3">{prescription.client?.fullName || "-"}</td>
                <td className="p-3">{prescription.medication?.name || "-"}</td>
                <td className="p-3">{new Date(prescription.validUntil).toLocaleDateString()}</td>
